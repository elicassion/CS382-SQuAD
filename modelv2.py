...
class Model(object):
    ...
    def ready(self):
        ...
        with tf.variable_scope("attention-2"):
            qc_att_2 = dot_attention(match, q, mask=self.q_mask, hidden=d,
                                   keep_prob=config.keep_prob, is_train=self.is_train)
            rnn = gru(num_layers=1, num_units=d, batch_size=N, input_size=qc_att_2.get_shape(
            ).as_list()[-1], keep_prob=config.keep_prob, is_train=self.is_train)
            att_2 = rnn(qc_att_2, seq_len=self.c_len)

        with tf.variable_scope("match-2"):
            self_att_2 = dot_attention(
                att_2, att_2, mask=self.c_mask, hidden=d, keep_prob=config.keep_prob, is_train=self.is_train)
            rnn = gru(num_layers=1, num_units=d, batch_size=N, input_size=self_att_2.get_shape(
            ).as_list()[-1], keep_prob=config.keep_prob, is_train=self.is_train)
            match_2 = rnn(self_att_2, seq_len=self.c_len)
        ...
        with tf.variable_scope("pointer"):
            ...
            logits1, logits2 = pointer(init, match_2, d, self.c_mask)
